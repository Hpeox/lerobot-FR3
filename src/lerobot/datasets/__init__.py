...
# Memmap conversion and training are useful even on a minimal deployment
# environment that does not install LeRobot's optional HuggingFace/AV stack.
# Keep the traditional exports when those extras are present, but do not make
# importing ``lerobot.datasets.acmt_act_memmap`` fail just because ``av`` is
# absent.  The H5 converter itself only needs h5py.
_dataset_extras_available = bool(is_package_available("datasets") and is_package_available("av"))

if _dataset_extras_available:
    ...
    # Keep the symbols imported by the map-style training loop available.
