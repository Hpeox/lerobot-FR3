...
class ACMTDPInferenceEngine(InferenceEngine):
    ...
    def __init__(
        ...
        elif policy_name == "acmt_actv2":
            # v2 is native ACT: it predicts a fresh 100-step chunk on every
            # control tick and owns a temporal ensembler.  This engine is the
            # legacy asynchronous 16/8 queue, so accepting v2 here would
            # silently execute the wrong action protocol.
            raise ValueError(
                "ACMT-ACTv2 uses ACMTACTV2InferenceEngine (native 100-step "
                "temporal ensemble), not the ACMT-DP 16/8 rolling engine"
            )
