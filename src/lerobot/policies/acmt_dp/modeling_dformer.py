...
class DropPath(nn.Module):
    """Parameter-free stochastic depth used by the vendored DFormerv2 blocks."""
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        ...
        random = keep_prob + torch.rand(shape, dtype=value.dtype, device=value.device)
        return value.div(keep_prob) * random.floor()
...
class RGBD_Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_e: torch.Tensor, split_or_not=False):
        ...
        if self.layerscale:
            x = x + self.drop_path(
                self.gamma_1 * self.Attention(self.layer_norm1(x), geo_prior, split_or_not)
            )
