...
from .configuration_acmt_dp_v3 import (
    DQ,
    FT300,
    GRIPPER_GPO,
    O_T_EE,
...
class ACMTDPV3Policy(PreTrainedPolicy):
    config_class = ACMTDPV3Config
    name = "acmt_dp_v3"

    def __init__(self, config: ACMTDPV3Config, **_: Any) -> None:
    ...
    def from_pretrained(cls, pretrained_name_or_path, *args, **kwargs):
        ...
        if config_path.is_file() and kwargs.get("config") is None:
            ...
            if raw.get("checkpoint_schema", "v3_temporal_center480") != "v3_temporal_center480":
                raise ValueError(
                    "ACMT-DP checkpoint schema is not v3_temporal_center480; "
                    "reconvert from a v3 best.pt checkpoint"
                )
        manifest_path = local_path / "conversion_manifest.json"
        requested_config = kwargs.get("config")
        if manifest_path.is_file():
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            if manifest.get("checkpoint_schema") not in (None, "v3_temporal_center480"):
                raise ValueError("ACMT-DP conversion manifest is not v3_temporal_center480")
            if manifest.get("policy_type") not in (None, "acmt_dp_v3"):
                raise ValueError("ACMT-DP conversion manifest is not for acmt_dp_v3")
    ...
    def train(self, mode: bool = True) -> ACMTDPV3Policy:
        super().train(mode)
        self.visual_encoder.eval()
