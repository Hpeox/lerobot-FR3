...
class ACMTACTV2Config(ACMTACTConfig):
    ...
    Fields inherited from :class:`ACMTACTConfig` intentionally retain the same
    ACT, tactile, action, and causal-generator defaults. Only the camera list
    and checkpoint ABI differ from v3.
    """

    checkpoint_schema: str = "acmt_actv2.v1"
    checkpoint_schema_version: int = 1
    camera_keys: tuple[str, str, str] = CAMERA_KEYS
    camera_names: tuple[str, str, str] = CAMERA_NAMES
    crop_params: dict[str, tuple[int, int, int, int]] = field(
        default_factory=lambda: dict(DEFAULT_CROP_PARAMS)
    )

    def __post_init__(self) -> None:
        # Keep the same CLI/JSON normalization as v3. In particular, draccus
        # can deserialize ``None`` as the literal string ``"None"``.
        ...
        # Call ACTConfig directly to retain upstream ACT validation while
        # bypassing ACMTACTConfig's intentional four-camera v3 checks.
    ...
    def _default_input_features(self) -> dict[str, PolicyFeature]:
        ...
        if self.tactile_source == "substitution":
            wrist_keys = tuple(
                camera for camera, name in zip(self.camera_keys, self.camera_names, strict=True)
    ...
    def validate_features(self) -> None:
        # The parent implementation checks the feature types/shapes against
        # self.camera_keys, so it remains valid for this three-camera list.
        ACMTACTConfig.validate_features(self)
