...
def test_rgb_normalizes_and_depth_preserves_z16_scale():
    rgb = prepare_fr3_array_for_policy("camera.cam1.rgb", np.full((480, 640, 3), 255, dtype=np.uint8))
    depth = prepare_fr3_array_for_policy(
        "camera.cam1.depth", torch.full((480, 640, 1), 1200, dtype=torch.uint16)
    )
...
def test_build_dataset_frame_preserves_numeric_array3d():
    features = fr3_observation_dataset_features(use_videos=False)
    values = {
