...
def test_processor_maps_fr3_rgbd_sources_without_overwriting_inputs() -> None:
    config = _config()
    processor = ACMTACTObservationProcessorStep(
        camera_keys=config.camera_keys,
        camera_names=config.camera_names,
    ...
    raw = {OBS_STATE: np.zeros(8, dtype=np.float32)}
    for index, camera in enumerate(config.camera_keys, start=1):
        raw[rgb_key(camera)] = np.full((480, 640, 3), index * 10, dtype=np.uint8)
        raw[depth_key(camera)] = np.full((480, 640), index * 1000, dtype=np.uint16)

    result = processor.observation(raw)
    expected_source_by_target = {
        "camera.cam1": (40, 4000),
        "camera.cam2": (30, 3000),
        "camera.cam3": (10, 1000),
        "camera.cam4": (20, 2000),
    }
    for target_camera, (rgb_value, depth_value) in expected_source_by_target.items():
        target_rgb = result[rgb_key(target_camera)]
        mean = torch.tensor(config.image_mean).view(3, 1, 1)
        std = torch.tensor(config.image_std).view(3, 1, 1)
        restored = target_rgb[0, :, 0, 0] * std[:, 0, 0] + mean[:, 0, 0]
        torch.testing.assert_close(restored, torch.full((3,), rgb_value / 255.0))
        assert result[depth_key(target_camera)][0, 0, 0, 0].item() == depth_value
